    ...
    async def create_text_chunks(
        ...
        try:    
            ...
            if not text or not text.strip():
                self.logger.warning("WARNING 빈 텍스트입니다.")
                return []
            ...
            while start_index < text_length and loop_count < 1000:  # 최대 1000번 반복 제한
                ...
                if chunk_content:
                    ...
                    self.logger.debug(f"STEP_CHUNK 청크 {len(chunks)}: 시작={start_index}, 끝={end_index}, 길이={len(chunk_content)}")
            ...
            if loop_count >= 1000:
                self.logger.warning(f"WARNING 청킹이 1000개 제한에 도달했습니다. 현재 {len(chunks)}개 청크 생성됨")
